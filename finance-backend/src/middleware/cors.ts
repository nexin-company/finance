...
export function getCorsHeaders(origin: string | null, config: CorsConfig = defaultCorsConfig): Record<string, string> {
	...
	if (config.exposedHeaders) headers['Access-Control-Expose-Headers'] = config.exposedHeaders.join(', ')
	if (config.maxAge) headers['Access-Control-Max-Age'] = config.maxAge.toString()
	return headers
}
...
export function getSecurityHeaders(isSwagger: boolean = false): Record<string, string> {
	...
	return {
		...
		'X-Frame-Options': isSwagger ? 'SAMEORIGIN' : 'DENY',
