...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${FINANCE_API_URL}/v1/ar/aging`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || 'Error al obtener aging' },
    ...
    console.error('Error en GET /api/finance/v1/ar/aging:', error);
    return NextResponse.json(
      { error: error.message || 'Error al obtener aging' },
      { status: 500 }
    );
  }
}
