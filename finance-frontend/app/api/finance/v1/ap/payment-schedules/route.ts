...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL(`${FINANCE_API_URL}/v1/ap/payment-schedules`);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || 'Error al obtener programación de pagos' },
    ...
    console.error('Error en GET /api/finance/v1/ap/payment-schedules:', error);
    return NextResponse.json(
      { error: error.message || 'Error al obtener programación de pagos' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${FINANCE_API_URL}/v1/ap/payment-schedules`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-API-Key': FINANCE_API_KEY,
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      return NextResponse.json(
        { error: errorData.message || 'Error al crear programación de pago' },
        { status: response.status }
      );
    }
    ...
    console.error('Error en POST /api/finance/v1/ap/payment-schedules:', error);
    return NextResponse.json(
      { error: error.message || 'Error al crear programación de pago' },
      { status: 500 }
    );
  }
}
