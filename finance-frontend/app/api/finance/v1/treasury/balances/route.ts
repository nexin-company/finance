...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${FINANCE_API_URL}/v1/treasury/balances`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || 'Error al obtener balances' },
    ...
    console.error('Error en GET /api/finance/v1/treasury/balances:', error);
    return NextResponse.json(
      { error: error.message || 'Error al obtener balances' },
      { status: 500 }
    );
  }
}
