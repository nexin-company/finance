...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${FINANCE_API_URL}/v1/treasury/cash-accounts`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || 'Error al obtener cuentas' },
    ...
    console.error('Error en GET /api/finance/v1/treasury/cash-accounts:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      return NextResponse.json(
        { error: errorData.message || 'Error al crear cuenta' },
        { status: response.status }
      );
    }
    ...
    console.error('Error en POST /api/finance/v1/treasury/cash-accounts:', error);
    return NextResponse.json(
